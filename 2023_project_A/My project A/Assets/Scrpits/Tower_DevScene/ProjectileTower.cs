...
public class ProjectileTower : MonoBehaviour
{
    ...
    public Transform firePoinst;                //발사 위치 설정
    public float timeBetweenShots = 1f;         //발사 간격
    private float shotCounter;                  //시간 설정


    private Transform target;                   //목표 타겠 설정
    public Transform launcherModel;             //모델 회전 시키키 위해서
    ...
    void Update()
    {
        ...
        if(shotCounter <= 0 && target != null)
        {
            ...
            firePoinst.LookAt(target);  //총알 발사는 Target를 바라본다.
            Instantiate(projectile, firePoinst.position, firePoinst.rotation);//총알을 생성한다.
        }
        if(thisTower.enemiesUpdate)         //적을 배열 LIst에서 검출했을때
        {
